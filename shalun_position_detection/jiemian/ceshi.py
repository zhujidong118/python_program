...
from PyQt5.QtWidgets import QDesktopWidget, QFileDialog, QLabel, QApplication
...
class MyMainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def slot_init(self):
        ...
        '初始化显示截取图像'
        show = cv2.imread("E:\\python\\shalun_position_detection\\pic_data\\jiequ.jpg")
        showImage = self.show_image(show)
        ...
        # '初始化显示零件图'
        show = cv2.imread("E:\\python\\shalun_position_detection\\pic_data\\nihe.png")
        showImage = self.show_image(show)
        showImage = showImage.scaled(self.label_3.width(), self.label_3.height(), Qt.KeepAspectRatio,
                                     Qt.SmoothTransformation)  # 按比例缩放
        self.label_3.setAlignment(Qt.AlignCenter)  # 居中显示
        self.label_3.setPixmap(QtGui.QPixmap.fromImage(showImage))

        '初始化显示零件标准图'
        show = cv2.imread("E:\\python\\shalun_position_detection\\pic_data\\lingjiantu.jpg")
        showImage = self.show_image(show)
    ...
    def show_jiequ(self):
        if self.camera_state == 1:
            ...
            cv2.imwrite("E:\\python\\shalun_position_detection\\pic_data\\jiequ.jpg", self.image)
            showImage = self.show_image(self.image)
            showImage = showImage.scaled(self.label_2.width(), self.label_2.height(), Qt.KeepAspectRatio,
                                         Qt.SmoothTransformation)  # 按比例缩放
            self.label_2.setAlignment(Qt.AlignCenter)  # 居中显示
    ...
    def on_toolButton_4_clicked(self):
        if self.jiequ_state == 0:
            ...
            if self.filename == "":
                self.label_4.clear()
                image = cv2.imread("E:\\python\\shalun_position_detection\\pic_data\\jiequ.jpg")
                image = cv2.resize(image, (0, 0), fx=0.5, fy=0.5)
                flag_compute, image, zuobiao = compute_contour(image, thresh=self.thresh, rho=self.rho,
                                                               theta=self.theta,
                                                               threshold=self.threshold,
                                                               minLineLength=self.minLineLength,
                                                               maxLineGap=self.maxLineGap)
                if flag_compute == True:
