...
from shalun_position_detection.position_detection import compute_contour, contour_interpolation, \
    save_zuobiao_xiangdui_txt, mkdir
...
class MyMainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        mkdir(".\\pic_data\\result_detection\\")
        mkdir(".\\pic_data\\wait_detection\\")  # 创建相应的文件夹
    ...
    def slot_init(self):
        ...
        '初始化显示零件标准图'
        show_biaozhun = cv2.imread(".\\pic_data\\lingjiantu.jpg")
    ...
    def show_jiequ(self):
        if self.camera_state == 1:
            ...
            cv2.imwrite(".\\pic_data\\wait_detection\\jiequ1.jpg", self.image)
            dialog = param2(self)
            result = dialog.exec_()
            if result == 1:
                ...
                cv2.imwrite(".\\pic_data\\wait_detection\\jiequ2.jpg", image)
    ...
    def on_toolButton_4_clicked(self):
        if self.jiequ_state == 0:
            ...
            if self.filename == "":
                self.label_4.clear()
                self.label_3.clear()
                image = cv2.imread(".\\pic_data\\wait_detection\\jiequ2.jpg")
                ...
                if flag_compute == 1:
                    ...
                    '比对计算'
                    image1 = contour_interpolation(zuobiao, "./pic_data/result_detection/")
                    ...
                    save_zuobiao_xiangdui_txt(
                        "./pic_data/result_detection/zuobiao_xiangdui.txt",
                        zuobiao)  # 保存轮廓像素相对坐标
...
class param2(QtWidgets.QDialog, Ui_Dialog_jiequ):
    def __init__(self, parent=None):
        ...
        image = cv2.imread(".\\pic_data\\wait_detection\\jiequ1.jpg")
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  # 视频色彩转换回RGB，这样才是现实的颜色
